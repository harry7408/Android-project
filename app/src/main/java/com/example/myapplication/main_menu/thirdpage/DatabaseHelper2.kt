...
class DatabaseHelper2 private constructor(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,null,
    DATABASE_VERSION) {

    companion object {


        const val DATABASE_NAME = "StudyList2.db"
        const val DATABASE_VERSION = 1
        const val TABLE_NAME = "Study_List2"
        const val COL1_ID = "_id"
        const val COL2_BOOK = "book"
        const val COL3_PUB = "publish"

        @Volatile
        private var instance: DatabaseHelper2? = null

        fun getInstance(context: Context) =
            instance ?: synchronized(DatabaseHelper2::class.java) {
                instance ?: DatabaseHelper2(context).also {
                    instance = it
                }
            }
