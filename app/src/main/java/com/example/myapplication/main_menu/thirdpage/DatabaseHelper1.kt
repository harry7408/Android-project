...
class DatabaseHelper1 private constructor(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,null,
    DATABASE_VERSION) {

    companion object {


        const val DATABASE_NAME = "StudyList1.db"
        const val DATABASE_VERSION = 1
        const val TABLE_NAME = "Study_List1"
        const val COL1_ID = "_id"
        const val COL2_BOOK = "book"
        const val COL3_PUB = "publish"

        @Volatile
        private var instance: DatabaseHelper1? = null

        fun getInstance(context: Context) =
            instance ?: synchronized(DatabaseHelper1::class.java) {
                instance ?: DatabaseHelper1(context).also {
                    instance = it
                }
            }
