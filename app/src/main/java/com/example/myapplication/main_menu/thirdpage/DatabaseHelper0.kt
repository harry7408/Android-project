...
class DatabaseHelper0 private constructor(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,null,
    DATABASE_VERSION) {

    companion object {


        const val DATABASE_NAME = "StudyList0.db"
        const val DATABASE_VERSION = 1
        const val TABLE_NAME = "Study_List0"
        const val COL1_ID = "_id"
        const val COL2_BOOK = "book"
        const val COL3_PUB = "publish"

        @Volatile
        private var instance: DatabaseHelper0? = null

        fun getInstance(context: Context) =
            instance ?: synchronized(DatabaseHelper0::class.java) {
                instance ?: DatabaseHelper0(context).also {
                    instance = it
                }
            }
