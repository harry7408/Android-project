...
class DatabaseHelper4 private constructor(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,null,
    DATABASE_VERSION) {

    companion object {


        const val DATABASE_NAME = "StudyList4.db"
        const val DATABASE_VERSION = 1
        const val TABLE_NAME = "Study_List4"
        const val COL1_ID = "_id"
        const val COL2_BOOK = "book"
        const val COL3_PUB = "publish"

        @Volatile
        private var instance: DatabaseHelper4? = null

        fun getInstance(context: Context) =
            instance ?: synchronized(DatabaseHelper4::class.java) {
                instance ?: DatabaseHelper4(context).also {
                    instance = it
                }
            }
